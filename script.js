...
const kegiatanData = [
    {
        ...
        description: "Kegiatan rutin tahunan untuk memperingati Hari Kemerdekaan Indonesia."
    },
    {
        gambar: "images/foto-lomba.jpg",
        judul: "Lomba Agustusan Tahun 2025",
        description: "Salah satu lomba anak yaitu pindah air dengan sendok."
    },
    {
        gambar: "images/momen-kebersamaan.jpg",
        judul: "Momen Kebersamaan di Evaluasi Kegiatan",
        description: "Momen indah di mana seluruh anggota organisasi berkumpul."
    }
];
        ...
        slidesHTML += `
            <div class="slide ${index === 0 ? 'aktif' : ''}">
                ...
                <div class="slide-content">
                    <h3>${item.judul}</h3><p>${item.description}</p>
                </div>
            </div>`;
...
async function loadRapatDariDrive() {
    try {
        ...
        for (let i = 1; i < baris.length; i++) {
            ...
            let thn = tglSplit[2] || tglSplit[0] || "2026"; if(thn.length > 4) thn = thn.substring(0,4); 
            let bln = namaBulanIndo[parseInt(tglSplit[1], 10) - 1] || "Semua";
...
/* ==========================================================================
   13. MODUL ARSIP DATA LOMBA REAL-TIME (SISTEM PAGINATION KAS KEUANGAN)
   ========================================================================== */
...
function tampilkanDataLombaKeTabel() {
    ...
    let html = pageData.map(item => {
        ...
        let btn = item.urlDrive ? `<button class="btn-cetak-mutasi" onclick="bukaPdfViewer('${item.urlDrive}')" style="height:34px; padding:0 12px; font-size:12px;"><i class="fa-solid fa-eye"></i> Lihat PDF</button>` : `<i>Tidak tersedia</i>`;
    ...
    if (totalHalaman > 1) {
        ...
        if (halamanLombaSaatIni === 1) navHTML = `<div style="text-align:right;"><button onclick="window.navLombaManual(1)" style="${styleBtn}">Halaman Selanjutnya ></button></div>`;
        else if (halamanLombaSaatIni === totalHalaman) navHTML = `<div style="text-align:left;"><button onclick="window.navLombaManual(-1)" style="${styleBtn}">< Halaman Sebelumnya</button></div>`;
        else navHTML = `<div style="display:flex; justify-content:space-between;"><button onclick="window.navLombaManual(-1)" style="${styleBtn}">< Halaman Sebelumnya</button><button onclick="window.navLombaManual(1)" style="${styleBtn}">Halaman Selanjutnya ></button></div>`;
        html += `<tr><td colspan="4" style="padding:15px; background:#f9f9f9; border-top:1px solid #eee;">${navHTML}</td></tr>`;
    }
    tbody.innerHTML = html;
    const info = document.getElementById('info-halaman-tabel'); if (info) info.textContent = `Halaman ${halamanLombaSaatIni} dari ${totalHalaman}`;
}
...
function tampilkanDataProposalKeTabel() {
    ...
    if (totalHalaman > 1) {
        let navHTML = ""; const styleBtn = "padding:8px 16px; background:#D32F2F; color:white; border:none; border-radius:4px; cursor:pointer; font-weight:bold; font-size:12px;";
        if (halamanProposalSaatIni === 1) navHTML = `<div style="text-align:right;"><button onclick="window.navProposalManual(1)" style="${styleBtn}">Halaman Selanjutnya ></button></div>`;
        else if (halamanProposalSaatIni === totalHalaman) navHTML = `<div style="text-align:left;"><button onclick="window.navProposalManual(-1)" style="${styleBtn}">< Halaman Sebelumnya</button></div>`;
        else navHTML = `<div style="display:flex; justify-content:space-between;"><button onclick="window.navProposalManual(-1)" style="${styleBtn}">< Halaman Sebelumnya</button><button onclick="window.navProposalManual(1)" style="${styleBtn}">Halaman Selanjutnya ></button></div>`;
        html += `<tr><td colspan="4" style="padding:15px; background:#f9f9f9; border-top:1px solid #eee;">${navHTML}</td></tr>`;
    }
    tbody.innerHTML = html;
    const info = document.getElementById('info-halaman-proposal'); if (info) info.textContent = `Halaman ${halamanProposalSaatIni} dari ${totalHalaman}`;
}
...
function tampilkanDataSuratKeTabel() {
    ...
    if (totalHalaman > 1) {
        let navHTML = ""; const styleBtn = "padding:8px 16px; background:#D32F2F; color:white; border:none; border-radius:4px; cursor:pointer; font-weight:bold; font-size:12px;";
        if (halamanSuratSaatIni === 1) navHTML = `<div style="text-align:right;"><button onclick="window.navSuratManual(1)" style="${styleBtn}">Halaman Selanjutnya ></button></div>`;
        else if (halamanSuratSaatIni === totalHalaman) navHTML = `<div style="text-align:left;"><button onclick="window.navSuratManual(-1)" style="${styleBtn}">< Halaman Sebelumnya</button></div>`;
        else navHTML = `<div style="display:flex; justify-content:space-between;"><button onclick="window.navSuratManual(-1)" style="${styleBtn}">< Halaman Sebelumnya</button><button onclick="window.navSuratManual(1)" style="${styleBtn}">Halaman Selanjutnya ></button></div>`;
        html += `<tr><td colspan="4" style="padding:15px; background:#f9f9f9; border-top:1px solid #eee;">${navHTML}</td></tr>`;
    }
    tbody.innerHTML = html;
    const info = document.getElementById('info-halaman-surat'); if (info) info.textContent = `Halaman ${halamanSuratSaatIni} dari ${totalHalaman}`;
}
